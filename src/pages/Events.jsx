...
export default function Events() {
    ...
    const [loadingMsg, setLoadingMsg] = useState("Loading events...");
    ...
    useEffect(() => {
        ...
        getEvents({
            ...
            onAttempt: (attempt) => {
                if (!mounted) return;
                if (attempt >= 2) setLoadingMsg("Waking server... (first load can take up to 1 minute)");
            },
        })
            .then((data) => {
                if (!mounted) return;
            ...
            .catch(() => {
                if (!mounted) return;
                setErr("Could not load events. Please refresh in a few seconds.");
            })
            .finally(() => {
                if (!mounted) return;
                setLoading(false);
